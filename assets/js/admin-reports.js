(function () {
  ...
  function history(host, row) {
    ...
    function copyText(copy) { return Object.keys(copy).map(function (key) { const value = Array.isArray(copy[key]) ? copy[key].join('\n') : copy[key]; return ({ title: '标题', summary: '摘要', highlights: '经营结论', signals: '分析', adviceSummary: '建议摘要', adviceHighlights: '重点关注事项', advice: '建议' })[key] + '：' + value; }).join('\n'); }
  ...
  function edit(host, row, refresh) {
    ...
    workspace.element.querySelector('form').addEventListener('submit', function (event) { event.preventDefault(); try { const input = Object.fromEntries(new FormData(event.currentTarget)); input.highlights = lines(input.highlights); input.adviceHighlights = lines(input.adviceHighlights); const saved = store.edit(row.id, row.version, row.revision, input); workspace.complete('保存完成 · ' + states[saved.status].name + '。'); } catch (error) { workspace.element.querySelector('[role="alert"]').textContent = error.message; } });
  }
