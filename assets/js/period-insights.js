(function () {
  ...
  function markup(c) {
    ...
    let html = '<section class="bulletin-section"><h3>完整自然月对照</h3><p class="field-hint">上月：' + v.previous.start + ' 至 ' + v.previous.end + '；上年同月：' + v.year.start + ' 至 ' + v.year.end + '。入出库比较月累计，月天数不同；价格按覆盖天数加权，库存取月末。</p><div class="table-scroll"><table class="data-table compact-table"><thead><tr><th>指标</th><th>本月</th><th>上月</th><th>环比变化量 / 率</th><th>上年同月</th><th>同比变化量 / 率</th></tr></thead><tbody>';
