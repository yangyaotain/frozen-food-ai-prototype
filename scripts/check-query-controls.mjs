...
assert.equal(changes.length, 4, 'affordance synchronization must not add queries on reset');
...
assert.match(analysis, /missing\.push\('截至周'\)/); assert.match(analysis, /missing\.push\('分析范围'\)/);
assert.match(analysis, /result = \{ valid: false \}/); assert.match(analysis, /2026-08-24/); assert.match(analysis, /value="4" selected/);
